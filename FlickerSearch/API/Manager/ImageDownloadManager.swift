...
//  FlickerSearch
//
//  Created by RAJESH KUMAR on 29/06/20.
...
final class ImageDownloadManager {
    ...
    lazy var imageDownloadQueue: OperationQueue = {
        ...
        queue.name = "com.flickr.imageDownloadQueue"
    ...
    func downloadImage(_ photo: Photo, indexPath: IndexPath?,
        ...
        if let cachedImage = imageCache.object(forKey: url.absoluteString as NSString) {
            ...
            if let operations = (imageDownloadQueue.operations as? [ImageOperation])?.filter({$0.imageUrl.absoluteString == url.absoluteString && $0.isFinished == false && $0.isExecuting == true }), let operation = operations.first {
                ...
                // create a new task to download image
    ...
    /*Function to reduce the priority of the network operation
     in case the user scrolls and an image is no longer visible*/
    func slowDownImageDownloadTaskFor(_ photo: Photo) {
        guard let url = photo.flickrImageURL() else {
            return
        }
        if let operatios = (imageDownloadQueue.operations as? [ImageOperation])?.filter({$0.imageUrl.absoluteString == url.absoluteString && $0.isFinished == false && $0.isExecuting == true }), let operation = operatios.first {
